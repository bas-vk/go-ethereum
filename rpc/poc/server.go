...
func suitableMethods(typ reflect.Type, reportErr bool) map[string]*handlerType {
	...
	for m := 0; m < typ.NumMethod(); m++ {
		...
		// Method can have 0 or a single 1 argument
...
func (server *Server) sendResponse(req *Request, reply interface{}, codec ServerCodec, err error) {
	resp := server.getResponse()
	// Encode the response header
	resp.ServiceMethod = req.ServiceMethod
	var werr error
	if err != nil {
		werr = codec.WriteErrorResponse(resp, 32101 /* RLP doesn't support negative numbers as JSON rpc uses*/, err)
	} else {
...
func (s *service) call(server *Server, mtype *handlerType, req *Request, argv, replyv reflect.Value, codec ServerCodec) {
	mtype.Lock()
	mtype.numCalls++
	mtype.Unlock()
	...
	if len(returnValues) > 0 {
		if e, ok := returnValues[len(returnValues)-1].Interface().(error); ok {
			server.sendResponse(req, nil, codec, e)
			return
		}
	}
	...
	if len(returnValues) > 0 {
		server.sendResponse(req, returnValues[0].Interface(), codec, nil)
		return
	}

	// no reply
	server.sendResponse(req, nil, codec, nil)
}
...
func (server *Server) ServeCodec(codec ServerCodec) {
	for {
		...
		if err != nil {
			...
			if req != nil {
				server.sendResponse(req, invalidRequest, codec, err)
...
type ServerCodec interface {
	...
	WriteErrorResponse(*Response, int, error) error
